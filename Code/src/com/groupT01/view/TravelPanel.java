...
public class TravelPanel extends Panel {
	Connection con;
	...
	public TravelPanel() {
		...
		JLabel hint = new JLabel("<html>\r\n" + "<body>\r\n" + "<p>Attention: <br/>0. SID: 旅游方案序号，GID: 导游序号，PID: 旅游路线序号"
				+ "<br/>1. SID 字段主键自增；<br/>2. 添加时仅需从弹出的表格中选择合适的 PID 和 GID 填入相应空格，其余内容填了也无效。<br/>"
	...
	public void MyEvent() {
		...
		update.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO 自动生成的方法存根
				GuidePop guidebox = new GuidePop();// 为跳转的界面
		...
		save.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				sing_TP.setGid(Integer.parseInt(table.getValueAt(table.getSelectedRow(), 1).toString()));
				sing_TP.setPid(Integer.parseInt(table.getValueAt(table.getSelectedRow(), 2).toString()));

				try {
					int result = dao.insert(sing_TP, con);
					if (result == 50) {
