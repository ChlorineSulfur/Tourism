...
import com.group82.dao.ForumDao;
import com.group82.model.Forum;
import com.group82.util.DataBase;

public class GuideForumPanel extends Panel {
	Connection con;
	...
	public GuideForumPanel() {
		...
		JLabel hint = new JLabel("<html>\r\n" + "<body>\r\n"
				+ "<p>Attention: <br/>1. Id 字段主键自增；<br/>2. 身份、用户名自动绑定，日期自动获取今日日期。<br/>&nbsp;&nbsp;&nbsp; 故最简便的方法仅需填入具体评论内容！<br/>"
	...
	public void MyEvent() {
		...
		save.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				sing_F.setIdentity("导游");
				// sing_F.setFname(table.getValueAt(table.getSelectedRow(),2).toString());
				sing_F.setFname(LOGIN.guide_login.getUsername());
		...
		del.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				String identity = table.getValueAt(table.getSelectedRow(),1 ).toString();
				String name_guide = table.getValueAt(table.getSelectedRow(),2).toString();
				if (id_key.equals(""))
					JOptionPane.showMessageDialog(null, "存在空对象！", "错误", JOptionPane.ERROR_MESSAGE);
				else {
					if(name_guide.equals(LOGIN.guide_login.getUsername())) {
						if (rowcount >= 0) {
						...
						JOptionPane.showMessageDialog(null, "无权限删除其他用户的论坛", "错误", JOptionPane.ERROR_MESSAGE);
					}
				}
